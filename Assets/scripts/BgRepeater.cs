...
public class BgRepeater : MonoBehaviour {
    ...
    private BoxCollider2D skybox;       //This stores a reference to the collider attached to the Ground.
    private float HorizontalLength;       //A float to store the x-axis length of the collider2D attached to the Ground GameObject.

    //Awake is called before Start.
    private void Awake()
    {
        //Get and store a reference to the collider2D attached to Ground.
        skybox = GetComponent<BoxCollider2D>();
        //Store the size of the collider along the x axis (its length in units).
        HorizontalLength = skybox.size.x;
    }
    ...
    private void RepositionBackground()
    {
        ...
        Vector2 skyOffSet = new Vector2(HorizontalLength * 2f, 0);
